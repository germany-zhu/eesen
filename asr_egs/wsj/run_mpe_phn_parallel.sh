...
#    data/lang_phn_test_tg data/train_tr95 $srcdir ${srcdir}_denlats || exit 1;
  ...
  steps/train_mpe_parallel.sh --cmd "$cuda_cmd" --num-iters 4 --acwt $acwt --do-smbr true --learn-rate $learn_rate \
    data/train_tr95 data/lang_phn $srcdir ${srcdir}_ali ${srcdir}_denlats ${srcdir}_mpe_lr${learn_rate}_parallel || exit 1;
