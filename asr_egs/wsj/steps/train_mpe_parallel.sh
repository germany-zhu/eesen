...
for f in $data/feats.scp $alidir/ali.1.gz $denlatdir/lat.scp $srcdir/final.nnet_parallel; do
...
nnet=$srcdir/$(readlink $srcdir/final.nnet_parallel || echo final.nnet_parallel);
...
while [ $x -le $num_iters ]; do
  ...
  nnet-train-mpe-sequential-end-end-parallel \
